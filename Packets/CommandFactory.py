...
from Packets.Messages.Server.ServerBoxShop import ServerBoxShop
...
class EndClientTurn(BSMessageReader):
    ...
    def decode(self):
        ...
        print("[RX] COMMAND - ", self.commandID)
    ...
    def process(self):
        if self.commandID == 500 or self.commandID == 517 or self.commandID == 535:
            ...
            print("[BOX]", "boxId has been handled!!! id -", self.commandID)
            ...
            print("[BOX]", "boxId has been handled!!! id -", self.commandID)
            print("[BOX]", "boxId has been handled!!! id -", self.commandID)
            ServerBoxShop(self.client, self.player).send()
            ...
            try:
                DataBase.replaceValueLD(self, 'profileIcon', self.player.profileIcon)
            except:
                print("Err")
            print("[INF] Change avatar")
        ...
        elif self.commandID == 511:
                self.player.errorID = 1
                self.player.errorText = "Server error/Ошибка сервера 43, сообщи pedikDev!!"
                SError(self.client, self.player).send()
            ...
            try:
                DataBase.replaceValueLD(self, 'namecolor', self.player.namecolor)
            except:
                print("Err")
            print("Command ID", self.commandID, "has been handled")
        ...
        elif self.commandID >= 0 and not 515:
            self.player.errorID = 1
            self.player.errorText = "it hasnt been implemented yet("
            SError(self.client, self.player).send()
            print("ID", self.commandID, "incorrent! Kicking player)))")
